...
namespace KERBALISM 
{
  ...
  public sealed class Configure : PartModule, IPartCostModifier, IPartMassModifier, IModuleInfo, ISpecifics
  {
    ...
    void Render_Panel(Panel p, ConfigureSetup setup, int selected_i, int setup_i)
    {
      ...
      {
        p.SetSection(setup.name, setup.desc);
      }
      else
      {
        p.SetSection(setup.name, setup.desc, () => Change_Setup(-1, selected_i, ref setup_i), () => Change_Setup(1, selected_i, ref setup_i));
      }

      // render details
      foreach(var det in setup.details)
      {
        p.SetContent(det.label, det.value);
      }
    }
