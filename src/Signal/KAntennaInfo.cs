...
namespace KERBALISM 
{
  public sealed class KAntennaInfo
  {
    public KAntennaInfo(Vessel v)
    {
      // initialize data
      type = new List<KAntennaType>();
      ...
      if (v.loaded)
      {
        ...
        foreach (Antenna a in Lib.FindModules<Antenna>(v))
        {
          if (!Settings.ExtendedAntenna || a.extended)
          {
            ...
            direct_cost += a.cost;
            if (a.type == KAntennaType.low_gain) indirect_cost += a.cost;
          }
          no_antenna = false;
        }
      }
