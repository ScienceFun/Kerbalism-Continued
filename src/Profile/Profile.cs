    ...
    public static void Parse()
    {
      ...
      if (Settings.Profile.Length > 0)
      {
        ...
        foreach(ConfigNode profile_node in Lib.ParseConfigs("Profile"))
        {
          ...
          {
            // log profile name
            Lib.Verbose(Lib.BuildString("using profile: ", Settings.Profile));
            ...
            foreach(ConfigNode rule_node in profile_node.GetNodes("Rule"))
            {
              ...
              {
                Lib.Verbose(Lib.BuildString("warning: failed to load rule (reason: ", e.Message, ")"));
              }
            }
            ...
            foreach(ConfigNode supply_node in profile_node.GetNodes("Supply"))
            {
              ...
              {
                Lib.Verbose(Lib.BuildString("warning: failed to load supply (reason: ", e.Message, ")"));
              }
            }
            ...
            foreach(ConfigNode process_node in profile_node.GetNodes("Process"))
            {
              ...
              {
                Lib.Verbose(Lib.BuildString("warning: failed to load process (reason: ", e.Message, ")"));
              }
            }

            // log info
            Lib.Verbose("supplies:");
            foreach(Supply supply in supplies) Lib.Verbose(Lib.BuildString("- ", supply.resource));
            if (supplies.Count == 0) Lib.Verbose("- none");
            Lib.Verbose("rules:");
            foreach(Rule rule in rules) Lib.Verbose(Lib.BuildString("- ", rule.name));
            if (rules.Count == 0) Lib.Verbose("- none");
            Lib.Verbose("processes:");
            foreach(Process process in processes) Lib.Verbose(Lib.BuildString("- ", process.name));
            if (processes.Count == 0) Lib.Verbose("- none");
        ...
        Lib.Verbose(Lib.BuildString("warning: profile '", Settings.Profile, "' was not found"));
      }
    }
